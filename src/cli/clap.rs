...
* `--share-track` and `--share-album` cannot be used with other options",
    ...
    .arg(device_arg())
    .arg(
      // TODO(clap-4): revisit semantics — was default_value_ifs with per-flag overrides
      // (seek -> "%f %s %t - %a %r", volume -> "%v% %f %s %t - %a", transfer -> "%f %s %t - %a on %d")
      // Simplified to a single default; callers get the right format for the common case.
      format_arg().default_value("%f %s %t - %a"),
    )
    .arg(
      Arg::new("toggle")
        .short('t')
...
be applied to every item found.",
    ...
    .visible_alias("l")
    // TODO(clap-4): revisit semantics — was default_value_ifs with per-flag overrides
    // (devices -> "%v% %d", liked -> "%t - %a (%u)", playlists -> "%p (%u)")
    // Simplified to a single default.
    .arg(format_arg().default_value("%t - %a (%u)"))
    .arg(
      Arg::new("devices")
        .short('d')
        .long("devices")
...
specify it.",
    ...
    .visible_alias("s")
    // TODO(clap-4): revisit semantics — was default_value_ifs with per-flag overrides
    // (tracks -> "%t - %a (%u)", playlists -> "%p (%u)", artists -> "%a (%u)",
    //  albums -> "%b - %a (%u)", shows -> "%h - %a (%u)")
    // Simplified to a single default.
    .arg(format_arg().default_value("%t - %a (%u)"))
    .arg(
      Arg::new("search")
